...
for line in file:
    ...
    signal.signal(signal.SIGALRM, handle_timeout)  # impostazione dell'handler per il segnale di timeout
    signal.alarm(5)  # timeout di 5 secondi
    # Start the strace command as a subprocess
    if '/' not in line:
        p = subprocess.Popen(['strace', f'/usr/bin/{line}'], 
                         stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setsid)
    else:
        p = subprocess.Popen(['strace', line], 
                         stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setsid)
    try: 
        ...
        os.killpg(os.getpgid(p.pid), signal.SIGINT) # Invia un segnale di interruzione al gruppo di processi della subprocess
